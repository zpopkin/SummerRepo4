...
data.frame(Price,Carat) -> Diamondsdata
Diamondsdata


ui <- fluidPage(
  ( titlePanel("Histograms")),
  h2("Diamonds: carat and price"),
  selectInput("vars", "Diamond variables", 
              choices = names(Diamondsdata)),
  plotOutput("plot"),
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(Diamondsdata, aes(y = .data[[input$vars]])) +
      geom_boxplot(fill = "darkred") +
      ggtitle("Diamond variable Histogram")
